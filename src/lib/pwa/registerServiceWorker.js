// EFATA777 V17 — conservative installability Service Worker registration
...
export function registerServiceWorker() {
  ...
  if (params.get("sw") === "off") {
    console.info("[PWA] EFATA777 V17: Service Worker pulado por sw=off.");
    return;
  }
  ...
  const exposeDiagnostic = async (registration = null) => {
    try {
      window.__PATROAI_PWA_STATUS__ = {
        version: "v17-installability-network-only",
  ...
  const register = async () => {
    try {
      ...
      console.info("[PWA] EFATA777 V17: Service Worker installability/network-only registrado:", registration.scope);
      ...
      registration.addEventListener?.("updatefound", () => {
        ...
        installing.addEventListener?.("statechange", () => {
          if (installing.state === "installed") {
            console.info("[PWA] EFATA777 V17: nova versão do SW instalada.");
          }
        });
      });
    } catch (error) {
      console.warn("[PWA] EFATA777 V17: Service Worker não registrado:", error?.message || error);
  ...
  navigator.serviceWorker.addEventListener?.("controllerchange", () => {
    console.info("[PWA] EFATA777 V17: controller atualizado. Recarregue uma vez se o botão de instalação ainda não aparecer.");
