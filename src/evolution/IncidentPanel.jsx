import React from "react";

export default function IncidentPanel({ incidents = [] }) {
  return (
    <article style={panelStyle}>
      <h3 style={titleStyle}>Incidentes</h3>
      {!incidents.length ? <p style={emptyStyle}>Nenhum incidente carregado.</p> : <div style={{ display: "grid", gap: 8 }}>{incidents.map((incident) => <div key={incident.incident_id || incident.id} style={itemStyle}><strong>{incident.title || "Incidente sem título"}</strong><div style={metaStyle}>{incident.severity || "medium"} • {incident.status || "open"}</div></div>)}</div>}
    </article>
  );
}

const panelStyle = { border: "1px solid rgba(148,163,184,0.25)", borderRadius: 16, padding: 14 };
const titleStyle = { margin: "0 0 10px", fontSize: 16 };
const emptyStyle = { margin: 0, opacity: 0.7 };
const itemStyle = { padding: 10, borderRadius: 12, background: "rgba(15,23,42,0.35)" };
const metaStyle = { marginTop: 4, fontSize: 12, opacity: 0.68 };
