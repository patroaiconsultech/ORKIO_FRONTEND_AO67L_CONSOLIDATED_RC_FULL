...
function normalizeDeliveryStatus(status) {
  ...
  if (["sent", "partial", "failed", "not_configured", "unknown", "disabled"].includes(value)) {
    return value;
  }
  return "unknown";
}
...
function deliveryStatusMeta(status) {
  ...
  const meta = {
    sent: {
      ...
      text: "Your message was registered and a confirmation email was sent.",
    },
    partial: {
      label: "Request registered",
      tone: "text-amber-200 border-amber-400/25 bg-amber-400/10",
      text: "Your message was registered. Email confirmation may take a few minutes.",
    },
    failed: {
      label: "Request registered",
      tone: "text-amber-200 border-amber-400/25 bg-amber-400/10",
      text: "Your message was registered, but email confirmation may be delayed. Use WhatsApp if it is urgent.",
    },
    not_configured: {
      label: "Request registered",
      tone: "text-amber-200 border-amber-400/25 bg-amber-400/10",
      text: "Your message was registered. Email delivery status is not available in this environment.",
    },
    disabled: {
      label: "Request registered",
      tone: "text-white/70 border-white/10 bg-white/5",
      text: "Your message was registered.",
    },
    unknown: {
      label: "Request registered",
      tone: "text-white/70 border-white/10 bg-white/5",
      text: "Your message was registered. Please keep the protocol number below.",
    },
  };
  return meta[normalized] || meta.unknown;
}

function extractContactReceipt(result) {
  const payload = result?.data && typeof result.data === "object" ? result.data : result;
  const emailDelivery = payload?.email_delivery && typeof payload.email_delivery === "object"
    ? payload.email_delivery
    : {};
  const contactId = payload?.id || emailDelivery?.contact_id || null;
  const status = normalizeDeliveryStatus(emailDelivery?.status);
  return {
    id: contactId,
    message: payload?.message || "Your request has been received. We will respond as soon as possible.",
    emailDelivery: {
      status,
...
export default function Contact() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await apiFetch("/api/public/contact", { method: "POST", body: payload });
      setReceipt(extractContactReceipt(result));
      setSent(true);
    } catch (err) {
  ...
  if (sent) {
    const statusMeta = deliveryStatusMeta(receipt?.emailDelivery?.status);
      ...
      <div className="min-h-screen bg-[#070910] text-white flex items-center justify-center">
        <div className="max-w-md px-4 text-center">
          ...
          <p className="mt-3 text-white/70">
            {receipt?.message || "Your request has been received. We will respond as soon as possible."}
            {isPrivacy && " For data privacy requests, the legal response timeframe may be up to 15 days."}
          </p>

          <div className={`mt-6 rounded-2xl border p-4 text-left ${statusMeta.tone}`}>
            <div className="text-xs font-bold uppercase tracking-[0.18em] text-white/50">Protocol</div>
            <div className="mt-1 font-mono text-sm text-white">
              {receipt?.id || "Registered"}
            </div>
            <div className="mt-4 text-xs font-bold uppercase tracking-[0.18em] text-white/50">Delivery status</div>
            <div className="mt-1 text-sm font-semibold">{statusMeta.label}</div>
            <p className="mt-2 text-xs leading-5 text-white/60">{statusMeta.text}</p>
          </div>

          <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-center">
            <Link to="/" className="inline-block rounded-xl bg-gradient-to-r from-violet-500 to-cyan-400 px-6 py-3 text-sm font-bold text-black hover:brightness-110">
              Back to Home
            </Link>
            <a
              href={whatsappLink}
              ...
              className="inline-block rounded-xl border border-emerald-500/25 bg-emerald-500/10 px-6 py-3 text-sm font-bold text-emerald-300 hover:bg-emerald-500/15"
            >
              WhatsApp Support
            </a>
          </div>
